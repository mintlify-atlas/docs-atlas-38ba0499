...
- **Raw data access** — `readData(uint16_t *x, uint16_t *y, uint8_t *z)` writes the same values into caller-supplied pointers.
...
- **FlexIO SPI** — on Teensy 4.x, `begin(FlexIOSPI)` is supported when the `FlexIOSPI` library is present.
